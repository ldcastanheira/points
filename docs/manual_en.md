...
tempo para especificar uma redução na velocidade de sua equipe. Veja como
adicionar períodos de férias aos seus projetos no arquivo spec.pts do database
db-manual. Ex.:
...
worthonly é uma flag na tabela de **planned** ou **unplanned** que evita que o
aviso "Task %s in %s file is worthless" seja mostrado quando uma tarefa que não
pertence ao projeto está presente no arquivo de **planned** ou **unplanned**.
Isto é particularmente interessante quando desejamos criar um projeto que é
subprojeto de um outro projeto, mas que usa os mesmos arquivos de planned e
unplanned. Como o subprojeto terá menos tarefas, esta flag evita que a mensagem
...
# Rule com funções complexas

O campo rule do projeto é quem seleciona as tarefas do projeto. Esta função
pode ser complexa o quanto se queira. Ela é escrita lua. Toda tarefa que está
presente no campo **tasks** e chamada para ser selecionada pela função. O
primeiro parâmetro é tarefa e todo e qualquer campo desta tarefa pode ser usado
...
Points é um programa "one way", enquanto ele está rodando, nenhuma entrada de
dados é permitida. Toda a entrada de dados é feita através de leitura de
arquivos. A medida que os arquivos vão sendo carregados, é permitido ao usuário
ir agregando informações as tarefas ou trocando o valor dos seus campos. Este
recurso com certeza será usado de diferentes maneiras entre os usuários de
Points, pois a forma de utilizá-lo não será a mesma entre os usuários. As
tarefas do meu projeto são importadas do bugzilla e lá eu não consigo
classificar as tarefas corretamente. Assim, eu utilizo o Points para me ajudar
a mapear porque motivo as tarefas não planejadas estão aparecendo. Considere a
tarefa abaixo:
...
Um outro recurso oferecido pelo Points é o uso de templates. Points permite que você utilize uma outra tarefa com template para evitar a repetição de campos. Ao utilizar uma tarefa como template, todos os campos não especificados serão clonados da tarefa template. Veja como usar um campo templane no arquivo db-manual/tasks-ok.pts, tarefa "task3-1".

Por fim, um recurso geralmente utilizado com o template é o campo "attach".
Este campo permite que uma tarefa faça parte de um grupo de tarefas, melhorando
principalmente a saída do comando "show". Veja como utilizar no arquivo
