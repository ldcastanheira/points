...
--#if not DATE_OBJECT_AFX then
	...
	local function date_getdobj(v)
		...
		local er = string.format("invalid date time value (%s)", tostring(v))
		return (o and o:normalize() or error(er)), r -- if r is true then o is a reference to a date obj
	end
--#end -- not DATE_OBJECT_AFX
...
--#if not DATE_OBJECT_AFX then
	...
	function date:__call(...)
		...
		elseif n == 0 then return (date_getdobj(TODAY or false))
